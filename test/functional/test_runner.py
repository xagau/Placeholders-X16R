...
`test/functional/test_framework/test_framework.py:PlacehTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/placeh_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/PlacehProject/Placeholders/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/PlacehProject/Placeholders/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_placehd):
        print("No functional tests to run. Wallet, utils, and placehd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if placehd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "placehd"]) is not None:
            print("%sWARNING!%s There is already a placehd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "PLACEHD" not in os.environ:
        os.environ["PLACEHD"] = build_dir + '/src/placehd' + exeext
        os.environ["PLACEHCLI"] = build_dir + '/src/placeh-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `placeh-cli help` (`rpc_interface.txt`).
