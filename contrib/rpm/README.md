...
The RPM spec file provided here is for Placeh-Core 0.12.0 and builds on CentOS
...
[Placeh git repository](https://github.com/PlacehProject/Placeholders) but are not part of
...
Currently only `Source10` is used, for the example `placeh.conf` file.
...
in the placeh git repository. At present only an SVG version of the Placeh
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the placeh
...
needed when 0.12.1 is released, a proper fix is already in the Placeh git
master branch.
...
idea to build Placeholder Core against a static version of that library compiled
...
The `placeh.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `placeh-server` package. At this point, most RPM based
...
The `placeh-server` package should have SELinux support. How to properly do
...
Placeh-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/placeh/). He has been
packaging Placeh for Fedora at least since 2012.
...
4. I renamed the `placeh` package that contains the Qt GUI to `placeh-core` as
that appears to be how the general population refers to it, in contrast to
`placeh-xt` or `placeh-classic`. I wanted to make sure the general population
