...
BOOST_FIXTURE_TEST_SUITE(asset_tx_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(asset_tx_not_valid) {
        ...
        // This should fail because 900 PLACEH doesn't have a destination
        CAssetTransfer assetTransfer("PLACEH", 100);
    ...
    BOOST_AUTO_TEST_CASE(asset_tx_valid_multiple_outs) {
        ...
        for (int i = 0; i < 10; i++) {
            CAssetTransfer asset2("PLACEH", 100);
    ...
    BOOST_AUTO_TEST_CASE(asset_tx_multiple_assets) {
        ...
        CAssetTransfer asset2("PLACEHTEST", 1000);
        CScript scriptPubKey2 = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset2.ConstructTransaction(scriptPubKey2);

        CAssetTransfer asset3("PLACEHTESTTEST", 1000);
        ...
        for (int i = 0; i < 10; i++) {
            // Add the first asset
            CAssetTransfer outAsset("PLACEH", 100);
            ...
            CAssetTransfer outAsset2("PLACEHTEST", 100);
            ...
            // Add the third asset
            CAssetTransfer outAsset3("PLACEHTESTTEST", 100);
        ...
        // The inputs are spending 3000 Assets (1000 of each PLACEH, PLACEHTEST, PLACEHTESTTEST)
        // The outputs are spending 100 Assets to 10 destinations (10 * 100 = 1000) (of each PLACEH, PLACEHTEST, PLACEHTESTTEST)
